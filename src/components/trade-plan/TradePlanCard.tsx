      ...
      <CardHeader>
        <div className="flex items-start justify-between">
          <div>
            ...
            <CardDescription>
              {plan.timeframe.charAt(0).toUpperCase() + plan.timeframe.slice(1)}{" "}
              • {plan.vehicle}
            </CardDescription>
          </div>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pb-6 border-b">
          ...
          <div>
            ...
            <p className="text-lg font-semibold">
              +{plan.expected_move_percent.toFixed(1)}%
            </p>
          </div>
          ...
          <div>
            ...
            <p className="text-lg font-semibold text-red-500">
              -{plan.max_loss_percent.toFixed(1)}%
            </p>
          </div>
          ...
          <div>
            ...
            <p className="text-lg font-semibold">
              ${plan.invalidation_price.toFixed(2)}
            </p>
          </div>
        </div>
        ...
        {(plan.option_dte_range || plan.option_delta_range) && (
          <div className="p-3 bg-blue-500/10 rounded-lg border border-blue-200 dark:border-blue-800">
            ...
            {plan.option_dte_range && (
              <p className="text-sm">
                DTE: {plan.option_dte_range[0]} - {plan.option_dte_range[1]}{" "}
                days
              </p>
            )}
            {plan.option_delta_range && (
              <p className="text-sm">
                Delta: {plan.option_delta_range[0].toFixed(2)} -{" "}
                {plan.option_delta_range[1].toFixed(2)}
              </p>
            )}
          </div>
        )}
        ...
        {plan.is_suppressed &&
          plan.suppression_reasons &&
          plan.suppression_reasons.length > 0 && (

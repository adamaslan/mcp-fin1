    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <p className="text-3xl font-bold">${riskData.total_value.toFixed(2)}</p>
            <p className="text-xs text-muted-foreground mt-1">{riskData.positions.length} positions</p>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium">Total Max Loss</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-3xl font-bold text-red-500">${riskData.total_max_loss.toFixed(2)}</p>
        ...
        <Card>
          ...
          <CardContent>
            <Badge className={`${riskLevelColors[riskData.overall_risk_level]} text-base`}>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">of total capital</p>
          </CardContent>
        </Card>
      </div>

      {/* Risk alert */}
      {riskData.overall_risk_level === 'CRITICAL' || riskData.overall_risk_level === 'HIGH' && (
        <Card className="bg-red-500/10 border-red-200 dark:border-red-800">
          <CardContent className="pt-6 flex gap-3">
            <AlertCircle className="h-5 w-5 text-red-500 flex-shrink-0 mt-0.5" />
            <div>
              <p className="font-semibold text-red-700 dark:text-red-400">High Portfolio Risk</p>
              <p className="text-sm text-red-700 dark:text-red-400 mt-1">
                Your portfolio risk exceeds recommended levels. Consider reducing position sizes or hedging.
              </p>
            </div>
          </CardContent>
        </Card>
            ...
            <table className="w-full text-sm">
              <thead className="border-b">
                <tr>
                  ...
                  <th className="text-right py-3 font-semibold">Current Value</th>
              ...
              <tbody>
                {riskData.positions.map((position, idx) => (
                  <tr key={idx} className="border-b last:border-b-0 hover:bg-muted/50">
                    ...
                    <td className="text-right font-medium">${position.current_value.toFixed(2)}</td>
                    <td className="text-right text-red-500">
                      ${position.max_loss_dollar.toFixed(2)} (-{position.max_loss_percent.toFixed(1)}%)
                    </td>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              {Object.entries(riskData.sector_concentration).map(([sector, percentage]) => (
                <div key={sector}>
      ...
      {(riskData as any).hedge_suggestions && (riskData as any).hedge_suggestions.length > 0 && (

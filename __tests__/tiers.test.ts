...
test('Pro tier should include all free tier features (or their upgraded versions)', () => {
  freeFeatures.forEach((feature) => {
    ...
    const hasUpgradedVersion = proFeatures.includes(feature.replace('_limited', '_full'));
...
test('Pro tier analysesPerDay > Free tier analysesPerDay', () => {
  assertTrue(
    TIER_LIMITS.pro.analysesPerDay > TIER_LIMITS.free.analysesPerDay
  );
});

test('Max tier analysesPerDay >= Pro tier analysesPerDay', () => {
  assertTrue(
    TIER_LIMITS.max.analysesPerDay >= TIER_LIMITS.pro.analysesPerDay
  );
});

test('Pro tier scansPerDay > Free tier scansPerDay', () => {
  assertTrue(
    TIER_LIMITS.pro.scansPerDay > TIER_LIMITS.free.scansPerDay
  );
});

test('Max tier scansPerDay >= Pro tier scansPerDay', () => {
  assertTrue(
    TIER_LIMITS.max.scansPerDay >= TIER_LIMITS.pro.scansPerDay
  );
});

test('Pro tier watchlistCount > Free tier watchlistCount', () => {
  assertTrue(
    TIER_LIMITS.pro.watchlistCount > TIER_LIMITS.free.watchlistCount
  );
});

test('Max tier watchlistCount >= Pro tier watchlistCount', () => {
  assertTrue(
    TIER_LIMITS.max.watchlistCount >= TIER_LIMITS.pro.watchlistCount
  );
});

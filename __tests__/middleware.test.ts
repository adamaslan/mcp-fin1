...
test("/dashboard/alerts is NOT a pro-only route", () => {
  ...
  assertFalse(
    isProOnlyRoute(req),
    "/dashboard/alerts should be max-only, not pro-only",
  );
});
...
test("/dashboard/portfolio is NOT a max-only route", () => {
  ...
  assertFalse(
    isMaxOnlyRoute(req),
    "/dashboard/portfolio should be pro-only, not max-only",
  );
});
...
test("Pro-only and Max-only routes are mutually exclusive", () => {
  const proRoutes = ["/dashboard/portfolio", "/dashboard/journal"];
...
const accessTests: AccessTestCase[] = [
  ...
  // Pro-only routes
  {
    ...
    tier: "free",
    expectAccess: false,
    expectRedirect: "/dashboard?tier_required=pro",
  },
  ...
  // Max-only routes
  {
    ...
    tier: "free",
    expectAccess: false,
    expectRedirect: "/dashboard?tier_required=max",
  },
  {
    path: "/dashboard/alerts",
    tier: "pro",

...
test.describe("Tier Gates - Free Tier", () => {
  test("portfolio link shows lock icon for free users", async ({
    authenticatedPage,
  }) => {
  ...
  test("journal link shows lock icon for free users", async ({
    authenticatedPage,
  }) => {
    const sidebar = new Sidebar(authenticatedPage);
    await authenticatedPage.goto("/dashboard");
  ...
  test("alerts link shows lock icon for free users", async ({
    authenticatedPage,
  }) => {
    const sidebar = new Sidebar(authenticatedPage);
    await authenticatedPage.goto("/dashboard");
  ...
  test("clicking locked portfolio shows upgrade prompt", async ({
    authenticatedPage,
    page,
  }) => {
    const sidebar = new Sidebar(authenticatedPage);
    ...
    const hasUpgradeIndicator = await upgradeMentioned
      .isVisible()
      .catch(() => false);
    expect(hasUpgradeIndicator || isPricingPage).toBe(true);
  });

  test("clicking locked journal shows upgrade prompt", async ({
    authenticatedPage,
    page,
  }) => {
    const sidebar = new Sidebar(authenticatedPage);
  ...
  test("clicking locked alerts shows upgrade prompt", async ({
    authenticatedPage,
    page,
  }) => {
    const sidebar = new Sidebar(authenticatedPage);
  ...
  test("free tier sees correct tier-locked features", async ({
    authenticatedPage,
    tierHelper,
  }) => {
  ...
  test("free tier accessible features are not locked", async ({
    authenticatedPage,
    tierHelper,
  }) => {
...
test.describe("Tier Gates - Pro Features (Free Tier)", () => {
  test("portfolio is pro-only feature", async ({
    authenticatedPage,
    tierHelper,
  }) => {
  ...
  test("journal is pro-only feature", async ({
    authenticatedPage,
    tierHelper,
  }) => {
  ...
  test("portfolio requires pro tier upgrade", async ({
    authenticatedPage,
    tierHelper,
  }) => {
  ...
  test("journal requires pro tier upgrade", async ({
    authenticatedPage,
    tierHelper,
  }) => {
    const tier = await tierHelper.getCurrentTier();
...
test.describe("Tier Gates - Max Features (Free Tier)", () => {
  test("alerts is max-only feature", async ({
    authenticatedPage,
    tierHelper,
  }) => {
  ...
  test("alerts requires max tier upgrade", async ({
    authenticatedPage,
    tierHelper,
  }) => {
    const tier = await tierHelper.getCurrentTier();

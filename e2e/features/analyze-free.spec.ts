...
test.describe("Analysis - Free Tier", () => {
  test("free users see only swing timeframe (not day/scalp)", async ({
    authenticatedPage,
    tierHelper,
  }) => {
    ...
    expect(timeframes.some((t) => t.toLowerCase().includes("scalp"))).toBe(
      false,
    );
  });

  test("free tier receives max 3 signals (not all 150+)", async ({
    authenticatedPage,
    tierHelper,
  }) => {
    const analyzePage = new AnalyzePage(authenticatedPage);
  ...
  test("valid symbols return analysis results", async ({
    authenticatedPage,
  }) => {
    const analyzePage = new AnalyzePage(authenticatedPage);
  ...
  test("analysis page shows free tier indicator", async ({
    authenticatedPage,
    tierHelper,
  }) => {
    const analyzePage = new AnalyzePage(authenticatedPage);
  ...
  test("free tier sees swing timeframe as default", async ({
    authenticatedPage,
  }) => {
    const analyzePage = new AnalyzePage(authenticatedPage);
    await analyzePage.goto("AAPL");
  ...
  test("free tier cannot access day timeframe", async ({
    authenticatedPage,
  }) => {
    const analyzePage = new AnalyzePage(authenticatedPage);
    await analyzePage.goto("AAPL");
  ...
  test("free tier cannot access scalp timeframe", async ({
    authenticatedPage,
  }) => {
    const analyzePage = new AnalyzePage(authenticatedPage);
    await analyzePage.goto("AAPL");
...
test.describe("Analysis - Free Tier Daily Limit (5/day)", () => {
  test("displays 5 analyses per day limit", async ({
    authenticatedPage,
    tierHelper,
  }) => {
    const limits = await tierHelper.getTierLimits();
  ...
  test("user can perform analysis within daily limit", async ({
    authenticatedPage,
  }) => {
    const analyzePage = new AnalyzePage(authenticatedPage);

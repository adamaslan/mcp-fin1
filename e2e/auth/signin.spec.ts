...
test.describe('Authentication - Sign In', () => {
  test('user can sign in with valid credentials', async ({ page, authHelper }) => {
  ...
  test('session persists across page reloads', async ({ authenticatedPage }) => {
  ...
  test('authenticated user can access protected routes', async ({ authenticatedPage }) => {
    ...
    const protectedRoutes = ['/alerts', '/scanner', '/analyze/AAPL', '/watchlist'];
  ...
  test('user button shows in header after sign-in', async ({ authenticatedPage }) => {
    await authenticatedPage.goto('/dashboard');

    // Verify user button/menu is visible
    const userMenu = authenticatedPage.locator('[data-testid="user-menu"], button:has-text("Sign out")');

...
test.describe('API: /api/mcp/trade-plan - Free Tier', () => {
  ...
  test('free tier does not receive day timeframe', async () => {
    ...
    const dayTrades = data.trade_plans.filter(
      (p: any) => p.timeframe?.toLowerCase().includes('day')
    );
  ...
  test('free tier does not receive scalp timeframe', async () => {
    ...
    const scalpTrades = data.trade_plans.filter(
      (p: any) => p.timeframe?.toLowerCase().includes('scalp')
    );
...
test.describe('API: /api/mcp/trade-plan - Timeframe Filtering', () => {
  ...
  test('free tier gets swing trades only', async () => {
    ...
    const swingTrades = data.trade_plans.filter(
      (p: any) => p.timeframe?.toLowerCase().includes('swing')
    );
  ...
  test('requested timeframe parameter may be ignored for free tier', async ({ request }) => {
  ...
  test('scalp timeframe not available for free tier', async ({ request }) => {
    ...
    const hasScalp = data.trade_plans?.some(
      (p: any) => p.timeframe?.toLowerCase().includes('scalp')
    );
